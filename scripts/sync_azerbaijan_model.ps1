...
if ($IncludeModel) {
    $artifacts.Add("azerbaijan_model.pkl")
}
...
foreach ($artifact in $jsonArtifacts) {
    ...
    if ($artifact -eq "azerbaijan_predictions.json" -and $parsed.Count -eq 0) {
        throw "Prediction artifact is empty: $source"
    }

    if ($artifact -eq "azerbaijan_metadata.json" -and -not $parsed.model_version) {
...
Write-Host "Azerbaijan model artifacts synced."
